...
class AdminBookingController
{
    ...
    public function get_all()
    {
        ...
        try {
            ...
            foreach ($bookings as &$b) {
                ...
                if ($b['status'] === 'Selesai' || $b['status'] === 'Dikembalikan') $badgeClass = 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300';
                ...
                $b['dates_format'] = "<b>" . date('d M Y', strtotime($b['start_date'])) . "</b> s/d <b>" . date('d M Y', strtotime($b['end_date'])) . "</b><br><span class='text-xs text-slate-400'>(" . $b['total_days'] . " Hari)</span>";
            }
    ...
    public function get_by_id($id)
    {
        ...
        if ($booking) {
            ...
            $booking['start_date_format'] = date('d F Y', strtotime($booking['start_date']));
            $booking['end_date_format'] = date('d F Y', strtotime($booking['end_date']));
    ...
    // Simpan Pemesanan Baru (Manual via Admin)
    public function store()
    {
        ...
        try {
            ...
            // Generate Kode Booking TRX-TAHUNBULANTANGGAL-RANDOM
            $booking_code = 'TRX-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -5));

            $sql = "INSERT INTO bookings (booking_code, customer_id, car_id, start_date, end_date, total_days, total_price, status, notes) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $db->prepare($sql);
            $stmt->execute([$booking_code, $customer_id, $car_id, $start_date, $end_date, $total_days, $total_price, $status, $notes]);

            // Jika status langsung disetujui / dipinjam, update status mobilnya
            if (in_array($status, ['Approve', 'Dipinjam'])) {
                $carStatus = ($status === 'Approve') ? 'Reservasi' : 'Disewa';
                $stmtCar = $db->prepare("UPDATE cars SET status = ? WHERE id = ?");
                $stmtCar->execute([$carStatus, $car_id]);
            }

            $db->commit();
            echo json_encode(['status' => true, 'message' => 'Pemesanan baru berhasil diinput!']);
        } catch (Exception $e) {
    ...
    public function update($id)
    {
        ...
        try {
            ...
            // Ambil data sewa lama untuk reset status mobil jika mobil diganti
            $oldBooking = $this->bookingModel->findById((int)$id);
            if ($oldBooking && $oldBooking['car_id'] != $car_id) {
                // Kembalikan status mobil lama ke "Tersedia"
            ...
            // Sesuaikan status mobil terkini
            $carStatus = 'Tersedia';
